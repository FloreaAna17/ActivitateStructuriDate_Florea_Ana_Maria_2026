...
void modifica_Nume(struct Student* s, char* noulNume) { //echivalent la POO cu setter, nu avem adresa in C
	if (strlen(noulNume) > 2) {
		free(s->nume);//sageata dereferentiaza si acceseaza
		(*s).nume = (char*)malloc(sizeof(char) * (strlen(noulNume) + 1));//s-> sau (*s) sunt echivalente
		strcpy(s->nume, noulNume);
	}
}//nu avem print in functii

void dezalocare(struct Student* s) {
	free(s->nume);
	s->nume = NULL;
}
char* compara_Studenti(struct Student student1, struct Student student2) {
...
int main() {
	...
	afisare(s);
	struct Student s2 = initializare(2, 1059, "Vasilescu Gigel", 8.3, 'A');//au aceeasi medie si se duce catre else
